...
*/
	...
	function __date ( $params = null ) {
		...
		if ( null === $params ) {
			...
			$resolve = __parseDateOriginToObj ( __newDateOrigin ( __parseDate ( $params ) ) );
		} else {
	...
	function __calendar ( $numberMonth = 1, $numberYear = 1970 ) {

		var $matrixDays = [ ];

		var $matrixWeek = $Week.map ( __weekLetter );
		
		var $currentMonth = __parseDateOriginToObj ( __newDateOrigin ( $numberYear+"-"+$numberMonth+"-1" ) );

		var $daysBeforeMonth  = __daysOfMonth ( ( Number ( $numberMonth ) - 1 ), $numberYear );

		var $weekCurrent = ( $currentMonth.w + 1 );

		var $dateBefore = ( $daysBeforeMonth - ( $weekCurrent - 2 ) );
		...
		for ( var $i = 1; $i <= 42; $i++ ) {
			...
			if ( $dateBefore > 1 && $dateBefore <= $daysBeforeMonth ) {
				$matrixDays.push ( $dateBefore );
				$dateBefore++;
			};

			if ( $i > $weekCurrent && $i <= ( $currentMonth.days + $weekCurrent ) ) {
				$matrixDays.push ( $countCurruent );
				$countCurruent++
			};

			if ( $i >= $currentMonth.days + $weekCurrent ) {
				$matrixDays.push ( $countAfter );
				$countAfter++;
			};
		};
...
//console.log ( $Date ( ).calendar ( 2, 2019 ) );